...
def load_whisper_model():
    ...
    if whisper_model is None:
        ...
        try:
             # Reduce threads to prevent contention on limited resources
             whisper_model = WhisperModel(WHISPER_MODEL_SIZE, device="cpu", compute_type="int8", cpu_threads=2)
...
# Start NLLB loading in background
...
async def transcribe_audio(
    ...
    try:
        ...
        logger.info(f"Transcribing {file.filename} | Lang: {language}")
...
async def translate_sarvam(req: TranslationRequest):
    ...
    try:
        ...
        # Return in a format the frontend expects (Direct text or object)
        # Frontend api.ts translateTextSarvam expects the raw string or result field.
        # Actually it returns response.text if it's a proxy, or data.translate_text.
...
async def export_pdf(req: PDFExportRequest):
    try:
        ...
        # Helper to register and set font
